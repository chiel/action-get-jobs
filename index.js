...
function getPackageDescriptions(packageNames) {
    return packageNames.map((name) => {
        ...
        return {
            dirName,
            name,
            path: require$$4__default$1["default"].resolve(process.cwd(), 'packages', dirName),
        };
    });
}
...
function run() {
    try {
        const packageNames = coreExports.getInput('packages', { required: true }).split(',');
        ...
        const jobs = Object.entries(resolvers).reduce((acc, [job, resolver]) => {
            ...
            if (!jobPackages.length)
                return acc;
            return Object.assign(Object.assign({}, acc), { [job]: jobPackages });
        }, {});
        console.log('jobs', jobs);
        coreExports.setOutput('jobs', JSON.stringify(jobs));
    }
    catch (err) {
        if (!(err instanceof Error)) {
            coreExports.setFailed('Something went horribly wrong');
            return;
        }
